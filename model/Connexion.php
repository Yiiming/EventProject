...
class Connexion
{
	...
	public function searchUser($etat, $begin, $row) {
		$resultat = $this->dbh->prepare("SELECT RESERVATION.ID AS ID, CIVILITE, RESERVATION.NOM AS NOM, PRENOM, MAIL, BIRTH, ADRESSE, CP, VILLE, TELEPHONE, CAMP.NOM AS CAMP, ARTEFACT.NOM AS ARTEFACT, DATE_CREATION, ETAT.NOM AS ETAT FROM RESERVATION, ETAT, ARTEFACT, CAMP WHERE RESERVATION.ETAT = ETAT.ID AND RESERVATION.ARTEFACT = ARTEFACT.ID AND RESERVATION.CAMP = CAMP.ID AND ETAT.ID = :etat ORDER BY DATE_CREATION DESC LIMIT $begin, $row");
		$resultat->bindValue(":etat", $etat, PDO::PARAM_INT);
		$resultat->execute();
		return $resultat;
	}
	...
	public function etatRefuse($id){
		...
		return $count = $resultat->rowCount();
	}
